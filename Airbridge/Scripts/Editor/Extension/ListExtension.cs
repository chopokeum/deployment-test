using System.Collections.Generic;
using System.Linq;
using JetBrains.Annotations;

// ReSharper disable UnusedType.Global
// ReSharper disable UnusedMember.Global
namespace Assets.Airbridge.Scripts.Editor.Extension
{
    public static class ListExtension
    {
        internal static List<string> AddRangeWithTrim(this List<string> self, [CanBeNull] string str, char separator)
        {
            if (string.IsNullOrEmpty(str)) return self;
            self.AddRange(
                str.Split(separator)
                    .Where(elem => !string.IsNullOrEmpty(elem))
                    .Select(elem => elem.Trim())
                    .Distinct()
                    .ToList()
            );

            return self;
        }
        
        internal static List<string> AddRangeWithTrim(this List<string> self, [CanBeNull] List<string> list)
        {
            if (list == null || list.Count == 0) return self;
            self.AddRange(
                list.Where(elem => !string.IsNullOrEmpty(elem))
                    .Select(elem => elem.Trim())
                    .Distinct()
                    .ToList()
            );

            return self;
        }
    
        // ReSharper disable once UnusedMethodReturnValue.Global
        internal static List<string> RemoveDuplicates(this List<string> self) => self.Distinct().ToList();
    }
}